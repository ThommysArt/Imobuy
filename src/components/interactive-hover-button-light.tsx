...
>(({ children, className, ...props }, ref) => {
  return (
    <button
      ref={ref}
      className={cn(
        "group relative w-auto cursor-pointer overflow-hidden rounded-full bg-white text-black p-3 px-6 text-center font-medium group-hover:border",
        className,
      )}
      {...props}
    >
      <div className="flex items-center gap-2">
        <div className="size-2 rounded-full bg-black transition-all duration-300 group-hover:scale-[100.8]"></div>
